...
public class MovementScript : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        camara.transform.position = new Vector3(transform.position.x, yHead ,transform.position.z)+offset;

        Debug.DrawRay(camara.transform.position, camara.transform.forward * 100f, Color.red);
        RaycastHit hit;
        if (Physics.Raycast(camara.transform.position, camara.transform.forward, out hit, 100f))
        {
            if (hit.rigidbody != null)
            {
                ...
                if (Input.GetMouseButtonDown(1))
                {
                    hit.rigidbody.useGravity = true;
                    pulsado = false;
                    hit.rigidbody.AddForce(camara.transform.forward * 1000f);
                }
            }
        }
        ...
        camara.transform.eulerAngles = new Vector2(rotHor, rotVer);
